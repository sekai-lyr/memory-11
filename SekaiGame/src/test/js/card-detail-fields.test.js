...
test("全部非N怪兽右键详情完整显示实际技能数量与时机", () => {
    ...
    for (const card of monsters) {
        ...
        for (const effect of card.effects) assert.ok(html.includes(effect.skillName), `${card.name}遗漏技能`);
        assert.equal((html.match(/<section class="detail-effect-field"/g) || []).length, card.effects.length, `${card.name}详情字段数量异常`);
    }
});
...
test("详情机械说明不会重复嵌套技能标签和触发时机", () => {
    ...
    assert.equal((html.match(/召唤成功时/g) || []).length, 1);
});
